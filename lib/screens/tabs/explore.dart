...
class ExplorePage extends StatelessWidget {
  ...
  if (response.statusCode == 200) {
    ...
    List<TodayEventModel> events = List<TodayEventModel>.from(jsonResponse['event_today'].map((eventJson) => TodayEventModel.fromJson(eventJson)));

    // Get the current date
    DateTime currentDate = DateTime.now();

    // Filter the events to get the ones happening today
    List<TodayEventModel> todayEvents = events.where((event) {
      DateTime eventDate = DateTime.parse(event.eventDate);
      return eventDate.year == currentDate.year &&
          eventDate.month == currentDate.month &&
          eventDate.day == currentDate.day;

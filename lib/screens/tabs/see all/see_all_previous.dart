...
class SeeAllPrevious extends StatefulWidget {
  const SeeAllPrevious({Key? key}) : super(key: key);

  @override
  State<SeeAllPrevious> createState() => _SeeAllPreviousState();
}

class _SeeAllPreviousState extends State<SeeAllPrevious> {
  ...
  Future<List<Widget>> fetchEndedEvents() async {
    ...
    try{
      ...
      if (response.statusCode == 200) {
        ...
        List<Widget> upcomingEventWidgets = endedEvents.map((event) {
          return Padding(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('All Previous Events'),
      ),

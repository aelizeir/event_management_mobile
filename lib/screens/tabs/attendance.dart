  ...
  Future<List<Widget>> fetchAttendedEvents() async {
    ...
    try{
      ...
      if (response.statusCode == 200) {
        ...
        List<AttendanceStatus> events = List<AttendanceStatus>.from(jsonResponse['attendanceStatus'].map((eventJson) => AttendanceStatus.fromJson(eventJson)));

        List<AttendanceStatus> attendedEvent = events.where((event) {
          return event.status == 'Attended';
        }).toList();

        List<Widget> attendedEventWidgets = attendedEvent.map((event) {
          return Container(
            ...
            return FutureBuilder<List<Widget>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  List<Widget> attendedEvents = snapshot.data!;
